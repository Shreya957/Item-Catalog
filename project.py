...
from detabaseSetup import Base, Category, Item
...
#	categories = session.query(Category).all()
#	items = session.query(Item).all()
#	return render_template('categories2.html', categories=categories, items = items, STATE = state)
	...
	if request.method == 'POST':
	    if login_session['username'] is not None:
			...
			for category_id in res:
				newItem = Item(name = request.form['itemname'], description = request.form['description'], cat_id = category_id.id)
				session.add(newItem)
				session.commit()
			conn.close()
			categories = session.query(Category).all()
